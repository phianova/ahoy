...
const SquawkCard = ({username, profilePic, postText, postImage, likes, user}) => {
    ...
    return (
        <div id="squawkcard" className='border  border-amber-500 rounded-lg p-3  overflow-hidden  mx-auto w-1/2 h-1/2 z-0'>
            <div className='top-3 left-3 pt-3 pl-3 flex '>
                {profilePic && <img src={profilePic} alt="Profile Picture" className='w-16 h-16 rounded-full mr-2 object-contain bg-gray-300' />}
                <p className={`mt-6 font-bold text-xl ${tradeWinds.className}`}><strong>{username}</strong></p>
                </div>
                <div className='p-3'>
                    ...
                    <p className="bg-yellow-100 p-1 w-32 mb-2 font-bold rounded-lg">{likeCount} People hooked</p>
                    
                    <div className='border-t-2 border-amber-500 pt-3'>
                    <button id="HookedButton" className='text-white bottom-2 right-3 bg-yellow-700 hover:bg-yellow-500 w-24 h-auto rounded-lg flex flex-row p-2 gap-2 shadow-md' onClick={handleIncrement} >Hooked
